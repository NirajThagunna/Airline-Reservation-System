...
<div class="container table-responsive">
	...
	<hr>
	<table class="table table-bordered table-hover table-info border-success align-middle">
		...
		<tbody>
			...
			<c:forEach var="flight" items="${listFlight}">
				<tr>
					...
					<td style="text-align: center;"><c:out value="${flight.getTravelDuration()}"></c:out></td>
					<td><c:out value="${flight.getAirportName()}"></c:out></td>
					<td><c:out value="${flight.getTicketPrice()}"></c:out></td>
					<td><a
						style="text-decoration: none; font-size: 18px;" href="bookTicket?flightNo=<c:out value='${flight.getFlightNo()}'/>">Book
							Ticket</a></td>
				</tr>
			</c:forEach>
			<!-- } -->
